...
from src.guesses.keyboards import guess_user_age_keyboard, report_keyboard, rate_user_keyboard
from src.guesses.schemas import Answers, Guess
...
async def react_for_user_guess(message: Message, user: User, session: AsyncSession, state: FSMContext) -> None:
    ...
    # await bot.(Answers.RATE_USER, user.user_id, message.message_id - 1)
