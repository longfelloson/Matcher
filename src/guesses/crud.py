...
from src.guesses.models import Guess
from src.guesses.schemas import Guess as GuessSchema
...
async def get_user_for_rate(user: User, user_rates: List[Rate], session: AsyncSession):
    ...
    conditions = [
        ...
        User.age.in_(config.GROUPS_AGES[user.preferred_age_group]),
    ]
...
async def get_total_user_guesses_points(user_id: int, session: AsyncSession) -> int | float:
    ...
    return points if points else ZERO_POINTS
