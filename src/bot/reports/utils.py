...
async def react_for_report(
    ...
    reported_user = await users_crud.get_user_by_id(report.reported, session)
    ...
    await send_reported_user_to_moderators(message, report, reported_user)


async def send_reported_user_to_moderators(
    message: Message,
    report: ReportSchema,
    reported: User,
):
