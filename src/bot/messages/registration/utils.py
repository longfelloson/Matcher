    ...
    match current_state:
        case RegistrationStates.name:
            ...
            await message.answer(
                RegAnswers.USER_AGE_SECTION, reply_markup=ReplyKeyboardRemove()
            )
        case RegistrationStates.gender:
            await state.set_state(RegistrationStates.name)
            await message.answer(
                RegAnswers.USER_NAME_SECTION, reply_markup=back_button_keyboard()
            )
        case RegistrationStates.preferred_gender:
            await state.set_state(RegistrationStates.gender)
            await message.answer(
                RegAnswers.USER_GENDER_SECTION, reply_markup=select_gender_keyboard()
            )
        case RegistrationStates.preferred_age_group:
            await state.set_state(RegistrationStates.preferred_gender)
            await message.answer(
                RegAnswers.PREFERRED_GENDER_SECTION,
        ...
        case RegistrationStates.location:
            ...
            await message.answer(
                RegAnswers.PREFERRED_AGE_GROUP_SECTION,
                reply_markup=select_age_group_keyboard(),
            )
        case RegistrationStates.photo:
            await state.set_state(RegistrationStates.location)
            await message.answer(
                RegAnswers.LOCATION_SECTION, reply_markup=select_location_keyboard()
            )
        case _:
            await state.clear()
