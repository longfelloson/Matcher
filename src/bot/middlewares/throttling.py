...
class ThrottlingMiddleware(BaseMiddleware):
    ...
    async def get_last_user_action_time(state: FSMContext) -> Optional[datetime]:
        ...
        return (
            datetime.fromisoformat(last_user_action_time_str)
    ...
    async def is_allowed_to_make_request(
        self, last_user_action_time: Optional[datetime]
    ) -> bool:
        """Проверяет, разрешено ли делать запрос на основе времени последнего действия"""
        if not last_user_action_time:
            return True
        return (
            datetime.now() - last_user_action_time
        ).total_seconds() >= self.rate_limit
    ...
    async def unblock_access_to_chat(self, event: Update, state: FSMContext) -> None:
        ...
        while True:
            ...
            if not unblock_time_isoformat or datetime.now() >= datetime.fromisoformat(
                data.get("unblock_time")
            ):
        ...
        await state.update_data(
            {
                ...
                "unblock_time": (
                    datetime.now() + timedelta(seconds=self.rate_limit)
                ).isoformat(),
            }
        )
