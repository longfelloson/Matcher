...
class UserStatusMiddleware(BaseMiddleware):
    async def __call__(
        ...
        user = await users_crud.get_user_by_id(user_id, data["session"])
        ...
        if user.status == UserStatus.blocked:
            return await event_message.answer(
                WarningAnswer.blocked_user, reply_markup=help_command_keyboard()
            )
