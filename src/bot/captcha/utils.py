...
def generate_captcha(emojis_limit: int = DEFAULT_CAPTCHA_EMOJIS_LIMIT) -> dict:
    """Generates a dictionary with emoji and their correctness"""
...
def encrypt_correctness(emoji_: str, correct_emoji: str) -> str:
    def get_captcha_correctness() -> CaptchaCorrectness:
        return (
            CaptchaCorrectness.CORRECT
            if emoji_ == correct_emoji
            else CaptchaCorrectness.INCORRECT
        )
...
def decrypt_correctness(encrypted_captcha_correctness: str) -> CaptchaCorrectness:
    return CIPHER.decipher(encrypted_captcha_correctness)


async def send_captcha(
    chat_id: Union[str, int], captcha_emojis: dict, hidden_correct_emoji: str
) -> None:
