...
async def get_user_name(message: Message, state: FSMContext):
    if message.text == BACK_BUTTON_EMOJI:
        ...
        return await message.answer(
            SectionAnswer.age, reply_markup=ReplyKeyboardRemove()
        )
    ...
    try:
        ...
        await message.answer(
            SectionAnswer.gender, reply_markup=select_gender_keyboard()
        )
    except ValidationError:
...
async def get_user_gender(message: Message, state: FSMContext):
    if message.text == BACK_BUTTON_EMOJI:
        await state.set_state(RegistrationState.name)
        return await message.answer(
            SectionAnswer.name, 
    ...
    try:
        ...
        await message.answer(
            SectionAnswer.preferred_gender,
...
async def get_user_preferred_gender(message: Message, state: FSMContext):
    if message.text == BACK_BUTTON_EMOJI:
        await state.set_state(RegistrationState.gender)
        return await message.answer(
            SectionAnswer.gender, reply_markup=select_gender_keyboard()
...
async def get_user_viewer_gender(message: Message, state: FSMContext):
    if message.text == BACK_BUTTON_EMOJI:
        await state.set_state(RegistrationState.preferred_gender)
        return await message.answer(
            SectionAnswer.preferred_gender,
    ...
    try:
        ...
        await message.answer(
            SectionAnswer.location, reply_markup=select_location_keyboard()
        )
    except ValidationError:
        await message.answer(IncorrectInputAnswer.buttons)
