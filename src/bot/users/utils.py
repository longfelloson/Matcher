...
async def get_users_for_view(
    ...
    specific_search_options, common_search_options, minimal_options = (
        get_search_options(
            rater_users_ids,
            rated_users_ids,
            guessed_users_ids,
    ...
    users = await users_crud.get_user_by_options(
        session, options=specific_search_options
    )
    if not users:
        users = await users_crud.get_user_by_options(
            session, options=common_search_options
        )
    if not users:
        users = await users_crud.get_user_by_options(session, options=minimal_options)
...
async def send_user_to_react(
    ...
    guessed_users_ids = await guesses_crud.get_guessed_users_ids(user.id, session)
    rated_users_ids = await rates_crud.get_rated_users_ids(user.id, session)
    rater_users_ids = await rates_crud.get_rater_users_ids(user.id, session)

    users_for_view = await get_users_for_view(
        rater_users_ids, rated_users_ids, guessed_users_ids, user, session
    )
    if not users_for_view:
        return await message.answer(
            Answer.no_user_for_guess, reply_markup=main_keyboard()
        )
    ...
    await state.set_data({"user_for_view_id": user_for_view.id})
...
async def send_user_to_view(
    ...
    if not chat_id and not message:
        raise ValueError(
            "Какой-то из аргументов должен быть заполнен для отправки пользователя!"
        )
